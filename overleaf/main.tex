...
\midrule
Vanilla & 0.461 & 0.386 & 0.716 & 0.713 \\
Finetuned & 0.455 & \textbf{0.480} & 0.681 & \textbf{0.725} \\
\bottomrule
\end{tabularx}
\end{table}
...
Several observations emerge from these results. First, the finetuned variant consistently outperforms vanilla, with the ``both'' configuration achieving 126 km median error compared to 133 km for vanilla, a 5\% improvement from Stage 0 pretraining.

Second, the ablation analysis reveals that concept-only predictions approach full model performance (137 km vs 126 km for finetuned), demonstrating that the concept bottleneck captures substantial geographic information. This validates our interpretability goal: predictions can be understood through concept activations without significant accuracy sacrifice.

Third, image-only predictions perform notably worse (154-222 km), indicating that the concept bottleneck provides regularization benefits beyond interpretability. The explicit semantic structure prevents the model from overfitting to low-level visual patterns.

Fourth, country-level accuracy exceeds 80\% across all configurations, showing reliable coarse localization even when fine-grained prediction is uncertain.
...
\centering
\small
\caption{Stage 2 geolocation performance on the out-of-distribution GeoGuessr dataset. Performance degrades compared to in-distribution data, with the vanilla variant showing better generalization.}
\label{tab:stage2_hf}
\begin{tabularx}{\columnwidth}{l>{\raggedright\arraybackslash}X*{6}{>{\centering\arraybackslash}X}}
\toprule
\textbf{Variant} & \textbf{Mode} & \textbf{Median (km)} & \textbf{Mean (km)} & \textbf{Cell Acc} & \textbf{City} & \textbf{Region} & \textbf{Country} \\
...
Out-of-distribution performance shows interesting patterns. Median error increases substantially (350-448 km vs 126-222 km), reflecting distribution shift between training and GeoGuessr imagery. The vanilla variant outperforms finetuned on this dataset, with the vanilla ``both'' mode achieving the best median error (349.9 km) and highest country-level accuracy (68.8\%). This suggests that Stage 0 pretraining may induce some overfitting to the training distribution, limiting generalization to out-of-distribution data.

The concept-only mode remains competitive with full cross-attention for both variants, further validating that learned concepts capture transferable geographic semantics. Country-level accuracy remains above 63\% across all configurations despite the distribution shift, indicating robust coarse localization. The image-only mode shows the largest performance degradation, particularly for vanilla (448.4 km median error), reinforcing the value of the concept bottleneck for generalization.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \label{fig:stage2_hf}
    \end{subfigure}
    \caption{Stage 2 geolocation results comparing model variants and ablation modes. The finetuned variant excels on in-distribution data, while vanilla shows better generalization to out-of-distribution GeoGuessr images.}
    \label{fig:stage2_results}
\end{figure}

\subsection{Ablation Analysis}

A comparative analysis of the three ablation modes for the finetuned variant reveals that the ``both'' configuration dominates across most metrics, with concept-only closely following. Image-only lags behind, particularly on city-level accuracy, suggesting that semantic concepts provide more robust localization cues than raw visual features alone.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{analysis/stage2_finetuned_radar.png}
...
A central question in concept bottleneck design is whether enforcing interpretability compromises prediction accuracy. Our results suggest the trade-off is minimal: concept-only predictions achieve within 10\% of full model performance (137 km vs 126 km median error on test data). This indicates that the 512-dimensional concept embedding captures sufficient geographic information for competitive geolocation, while providing the substantial benefit of human-interpretable intermediate representations.

Stage 0 contrastive pretraining provides clear benefits on in-distribution data, reducing median error from 133 km to 126 km. However, results on the GeoGuessr dataset reveal a generalization gap: the vanilla model without pretraining actually performs better out-of-distribution. This suggests a tension between optimizing for the training distribution and learning broadly transferable representations. Future work could explore regularization techniques or multi-domain pretraining to improve generalization.

The hierarchical organization of child and parent concepts proves valuable for learning structured representations. The finetuned variant shows substantially improved parent concept accuracy (48.0\% vs 38.6\%), indicating that the contrastive pretraining helps establish correct semantic relationships. The consistency losses ensure that child predictions aggregate correctly to parent levels, providing multiple granularities of interpretable output.

The per-country clustering approach for geocell generation effectively allocates prediction granularity according to data density. Dense regions like Western Europe and East Asia receive finer cells, enabling more precise predictions where data supports it, while maintaining global coverage through coarser cells in sparse regions.

Several limitations warrant mention. First, our dataset of 43,000 images, while diverse, may not capture the full variety of global street view imagery. Second, the concept vocabulary was derived from the training data and may miss important geographic indicators present in other regions. Third, while cross-attention provides spatial interpretability, the concept embeddings themselves remain high-dimensional vectors that require further analysis to fully interpret.
...
This work presented a concept-aware geolocation system that predicts geographic coordinates through interpretable semantic reasoning. By combining hierarchical concept bottleneck models with vision-language foundations, we achieve competitive geolocation accuracy (126 km median error on test data, 352 km on out-of-distribution GeoGuessr images) while providing human-understandable explanations through concept activations and spatial attention maps.
