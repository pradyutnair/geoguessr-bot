...
Out-of-distribution performance shows interesting patterns. The finetuned variant outperforms vanilla on this dataset, with the finetuned ``both'' mode achieving the best median error (349.9 km) and highest country-level accuracy (68.8\%). This demonstrates that Stage 0 pretraining provides transferable representations that generalize to out-of-distribution data.

Comparing variants within each ablation mode, the finetuned model consistently outperforms vanilla across all modes: for the ``both'' mode (349.9 km vs 391.5 km, a 10.6\% improvement), concept-only (381.8 km vs 417.6 km, 8.6\% improvement), and image-only (387.0 km vs 448.4 km, 13.7\% improvement). This consistent advantage across all modes validates that Stage 0 pretraining provides broadly transferable representations regardless of the inference configuration. Country-level accuracy remains above 63\% across all configurations despite the distribution shift, indicating robust coarse localization. The baseline GeoCLIP model achieves significantly higher median error (1015.8 km) and lower threshold accuracies across all scales, demonstrating the advantage of our concept-aware approach.
...
Stage 0 contrastive pretraining provides clear benefits on both in-distribution and out-of-distribution data, reducing median error from 133 km to 126 km on test data and from 391.5 km to 349.9 km on the GeoGuessr dataset. This demonstrates that the contrastive pretraining learns broadly transferable representations that generalize effectively to out-of-distribution imagery. The consistent improvement across both evaluation settings validates the value of domain-specific pretraining for geolocation tasks.
